...
@section('content')
    <div class="max-w-7xl mx-auto py-6 animate-fade-in-up">
            ...
            <div class="flex gap-2 align-items-center">
                @if (Auth::user()->canPerformActions())
                    ...
                    @if ($template_titles->count())
                        <form method="GET" action="{{ route('kimia.create') }}"
                            class="d-flex align-items-center gap-2 bg-white border border-success rounded px-2 py-1 shadow-sm"
                            style="max-width: 340px;">
                            <select name="template_title" id="template_title" class="form-select form-select-sm"
                                style="min-width:220px; max-width:320px;" required>
                ...
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <form method="GET" action="{{ route('kimia.index') }}"
                                class="d-flex align-items-center gap-2">
                                <select name="approval" class="form-select" style="min-width: 220px;"
                                    onchange="this.form.submit()">
                                    <option value="">📋 Semua Approval</option>
                                    <option value="pending" {{ request('approval') == 'pending' ? 'selected' : '' }}>⏳ Belum
                                        Lengkap Semua</option>
                                    <option value="completed" {{ request('approval') == 'completed' ? 'selected' : '' }}>✅
                                        Sudah Lengkap Semua</option>
                                    <option value="technician" {{ request('approval') == 'technician' ? 'selected' : '' }}>
                                        🔬 QA Lab. Technician (Belum Approve)</option>
                                    <option value="staff" {{ request('approval') == 'staff' ? 'selected' : '' }}>👥 QA
                                        Staff (Belum Approve)</option>
                                    <option value="supervisor" {{ request('approval') == 'supervisor' ? 'selected' : '' }}>
                                        👨‍💼 QA Supervisor (Belum Approve)</option>
                                </select>
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 border">
                <form method="GET" action="" class="row g-3 align-items-end">
                    <div class="col-md-3">
                        ...
                        <input type="text" name="search" placeholder="Judul, No Form, atau Tanggal..."
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="group_title" class="form-select">
                            ...
                            @foreach ($titles as $title)
                                <option value="{{ $title }}" {{ $group_title == $title ? 'selected' : '' }}>
                                    {{ $title }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        @if ($group_title)
            <div class="alert alert-info py-2 mb-3 animate-fade-in-up">Menampilkan data untuk judul:
                <b>{{ $group_title }}</b></div>
        @endif
                ...
                <tbody>
                    @forelse($forms as $form)
                        <tr class="hover:bg-yellow-50 cursor-pointer"
                            onclick="window.location='{{ route('kimia.show', $form) }}'"
                            data-form-id="{{ $form->id }}" data-entry-count="{{ $form->entries->count() }}"
                            data-approval-count="{{ $form->signatures->where('status', 'accept')->count() }}">
                            <td class="px-4 py-2">{{ $loop->iteration + ($forms->currentPage() - 1) * $forms->perPage() }}
                            </td>
                            <td class="px-4 py-2">{{ $form->title }}</td>
                            <td class="px-4 py-2">{{ $form->no }}</td>
                            <td class="px-4 py-2">{{ $form->tanggal->format('d/m/Y') }}</td>
                            <td class="px-4 py-2 flex gap-2">
                                <a href="{{ route('kimia.show', $form) }}"
                                    class="text-blue-600 hover:underline">Lihat</a>
                                @if (Auth::user()->canPerformActions())
                                    <a href="{{ route('kimia.edit', $form) }}"
                                        class="text-yellow-600 hover:underline">Edit</a>
                                @endif
                                <a href="{{ route('kimia.export', $form) }}"
                                    class="text-green-600 hover:underline">Export Excel</a>
                                @if (Auth::user()->canPerformActions())
                                    <form action="{{ route('kimia.destroy', $form) }}" method="POST"
                                        onsubmit="return confirm('Yakin hapus?')" onclick="event.stopPropagation()">
                                        @csrf @method('DELETE')
            ...
            <div class="d-flex align-items-center gap-2 p-2 bg-white rounded shadow-sm border" style="min-width:260px;">
                ...
                <nav>
                    <ul class="pagination pagination-sm mb-0">
                        ...
                        @else
                            <li class="page-item"><a class="page-link bg-warning text-primary border-0 fw-bold"
                                    href="{{ $forms->previousPageUrl() }}">&laquo;</a></li>
                        @endif
                        @foreach ($forms->getUrlRange(1, $forms->lastPage()) as $page => $url)
                            <li class="page-item {{ $page == $forms->currentPage() ? 'active' : '' }}">
                                <a class="page-link {{ $page == $forms->currentPage() ? 'bg-primary text-white border-primary' : 'bg-light text-primary border-0' }} fw-bold"
                                    href="{{ $url }}">{{ $page }}</a>
                            </li>
                        @endforeach
                        @if ($forms->hasMorePages())
                            <li class="page-item"><a class="page-link bg-warning text-primary border-0 fw-bold"
                                    href="{{ $forms->nextPageUrl() }}">&raquo;</a></li>
                        @else
    ...
    <script>
        ...
        `;
                document.querySelector('.max-w-7xl').insertBefore(alertDiv, document.querySelector('.max-w-7xl')
                    .firstChild);
